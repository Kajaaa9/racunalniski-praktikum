...
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amsthm} % tu notri definirana ukaza theoremstyle in newtheorem
\usepackage{booktabs} % da deluje \toprule, \midrule, \bottomrule
\usepackage{xcolor}
...
\newcommand{\pojem}[1]{\emph{\color{purple}#1}} % #1 predstavlja kaj je v prvem {} po uporabi tega ukaza v dokumentu

\newenvironment{magic}[3]{
   \begin{table}[!ht]
      \centering
      \caption{#2}      
      \label{#3} % !! nujno za ukazom caption !!
      \large
      \begin{tabular}{|*{#1}{c|}}\hline %drugi oklepaj za to da določimo stevilo stolpcev in poravnave
}{
      \end{tabular}
      \normalsize
   \end{table}
...
\begin{definicija}
   ...
   \begin{equation}
      % oznaka: eq:numbers  - !nujno enolične-ne uporabi dveh istih
...
% slike na 150% originalne velikosti. % scale=1.5
\begin{figure}[ht] %postavi sliko tja, ce to ne gre jo postavi na zacetek naslednje strani
   \centering
...
%
\begin{magic}{4}{Dürerjev magični kvadrat $4\times 4$}{table:durner}
   16 &  3 &  2 & 13 \\\hline
    5 & 10 & 11 &  8 \\\hline
    9 &  6 &  7 & 12 \\\hline
    4 & 15& 14 &  1 \\\hline
...
(glej sliko \ref{fig:durer} za fotografijo) vsebuje magični kvadrat reda 4.
% sklic: fig:sagrada
...
\begin{proof}[Dokaz izreka]
      ...
      števil (glej \eqref{eq:numbers} na strani \pageref{eq:numbers}) enaka  
         % pri enačbah za sklic uporabljamo \eqref, ne \ref; rabimo imeti \usepackage{amsmath}
         % za številko strani uporabimo \pageref
      ...
      enaka številu $\m$. % sklica: eq:numbers, eq:numbers
\end{proof}
...
Kvadratu v tabeli \ref{table:durner} ustrezata konstanti $A=20$ in $D=1$.
...
180 stopinj okrog središča, kvadrat iz tabele \ref{teble:table:mag3} pa je mogoče dobiti
