...
% Namig: z iskanjem po datotekah (Ctrl+Shift+F).
% poiščite niz `{definicija}' ali niz `{izrek}'.
\newtheorem{definicija}{Definicija}
\newtheorem{izrek}{Izrek}

\usepackage{tikz}       % za tikz-ovo knjiznico
\usetikzlibrary{math}   % da lahko rišemo matematicne zadeve

\usepackage{pgfplots}
\usepgfplotslibrary{external}

\usepackage{array}      % dodatne moznosti za tabele (npr. poravnava stolpcev)

% Definicija novih ukazov v preambuli
...
% Naloga 1.3.5: pripravite kazalo vsebine. (\tableofcontents)
