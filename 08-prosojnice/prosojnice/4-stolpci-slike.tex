\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	\begin{columns}
		\begin{column}{0.55\textwidth}
			  \begin{itemize}[<+->]  % <+-> ... da se odpira postopno
		...
		\begin{column}{0.45\textwidth}
			  \centering 									  %      ... da se celoten stolpec poravna
			  \includegraphics<1>[width=50mm]{slike/fig-1.png}%  <1> ...pove na kateri podprosojnici naj bo slika
			  \includegraphics<2>[width=50mm]{slike/fig-2.png}%   %  ... na koncu mora biti zato da so "vsi include graphics v isti vrstici" drugace gre vsak v svojo in je zato vsak malenkost nizje od prejsnjega
	...
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			...
			\begin{tikzpicture}
				...
				% Definicija točke T
				\coordinate [label={[blue, above left]:$T$}] (T) at (0,{\t});		% to definira na katerem mestu bomo tocko narisali, ime točke
				% Risanje točke T
				\fill[blue] (T) circle (2pt);										% to pa nariše točko
				% Premica p
				\draw[blue, very thick] (-2,0) -- (2,0) node[right] {$p$};			% nariše crto, doda ozako p
				\pause
				% Definicija pomožne točke A' in risanje krožnega loka k, ki se začne v A'
				\coordinate (A') at ({-\tt},{\td});									% kje definiramo točko A`
				\draw[gray, thin] (A') arc[start angle=210, end angle=330, radius=2] node[right] {\scriptsize $k$};
					% Točka A'   ! ne rabimo
					% \coordinate [label=above left:{\scriptsize $A'$}] (A') at ({\t},0);
					% \draw (A') circle (1pt);
				\pause
				% Točka A
				\coordinate [label=below left:{\scriptsize $A$}] (A) at ({-\t},0); % najprej definiramo polozaj tocke A
				\draw (A) circle (1.5pt);
				...
				\coordinate (T') at ({-1.7*\td},{\tt});
					%\coordinate [label=below right:{\scriptsize $T'$}] (T') at ({-\td},{\tt});
					%\draw (T') circle (1.5pt);																% node ... ime loka-ce na zacetku na prvi tocki, ce na koncu na zadnji tocki loka
				\draw [grey, thin] (T') node[right] {\scriptsize $m$} arc[start angle=60, end angle=-60, radius=2]; 		% !!! nujno ; na koncu !!!
				\pause

				% Naloga 3.4.3.: Definirajte točko T'' in narišite lok n z oznako.
				\coordinate (T'') at ({1.7*\td},{\tt});
				\draw [grey, thin] (T'') node[right] {\scriptsize $n$} arc[start angle=120, end angle=240, radius=2]; 		% !!! nujno ; na koncu !!!
				\pause
...
\begin{frame}{Graf funkcije s TikZ}
	...
	\begin{tikzpicture}
		...
		\addplot[red, thick]{2^(x - 3)/2^3 + 1};			% ! na koncu podpičje
